...
const Education = ({ education, isOwnProfile, onAdd, onEdit }) => {
    return (
                    ...
                    <div key={index} className="flex gap-4 group relative">
                        <div className="flex-shrink-0 mt-1">
                            <div className="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center">
                                <span className="material-icons text-slate-500 dark:text-slate-400">school</span>
                            </div>
                        </div>
                        <div className="flex-grow pb-6 border-b border-slate-100 dark:border-slate-700/50 group-last:border-none group-last:pb-0">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h3 className="text-base font-bold text-slate-900 dark:text-white">{edu.school}</h3>
                                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{edu.degree} • {edu.fieldOfStudy}</p>
                                </div>
                                {isOwnProfile && (
                                    <button
